...
class TrafficSlicing(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        actions = [
            ...
            )  # action object: the entire packet will be forwarded to the Ryu controller for further processing
        ]
    ...
    def _packet_in_handler(self, ev):
        ...
        if dst in self.mac_to_port[dpid]:  # if there's a match in the mac_to_port table
            out_port = self.mac_to_port[dpid][
                dst
            ]  # pick the out_port where to sand the packet
            actions = [
                datapath.ofproto_parser.OFPActionOutput(out_port)
            ]  # create an OFPActionOutput object with the picked port
            match = datapath.ofproto_parser.OFPMatch(
                eth_dst=dst
            )  # create a match object that matches with a the read Ethernet destination
